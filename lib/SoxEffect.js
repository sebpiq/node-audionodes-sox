...
_.extend(SoxEffect.prototype, {
  ...
  _tick: function() {
    ...
    else block = new AudioBuffer(this.format.numberOfChannels, BLOCK_SIZE, this.format.sampleRate) 
    return block
  },
    ...
    if (!this._pullingBlocksLock && !this._ended) {
      ...
      async.whilst(
        ...
        function(next) {
          ...
          if (block) {
            self._blocks.push(AudioBuffer.fromArray(block, self.format.sampleRate))
            next()
