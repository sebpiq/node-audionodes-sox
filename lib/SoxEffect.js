...
var SoxEffect = module.exports = function(context, opts) {
  if (!opts.hasOwnProperty('filename')) throw new Error('missing option filename')
  AudioNode.call(this, context, 0, 1)
  var self = this
    // full command w/ opts : sox input.wav -t raw -L -e signed -r 44100 -b 16 -c 2 - EFFECT [ARGS]
    , args = [opts.filename, '-t', 'raw', '-L', '-e', 'signed']
  ...
  var onFormatReceived = function(err, format) {
    if (err) return process.nextTick(function() { self.emit('error', err) })
    self.format = format
    ...
    self.process.stderr.on('data', function(data) {
      ...
      if (msg.indexOf('sox WARN') === 0) debug(msg)
  ...
  if (opts.inputRaw) {
    _.defaults(opts.inputRaw, {
      endianness: 'LE',
      signed: true
    })
    args = ['-t', 'raw',
      {'LE': '-L', 'BE': '-B'}[opts.inputRaw.endianness],
      '-e', opts.inputRaw.signed ? 'signed' : 'unsigned',
      '-r', opts.inputRaw.sampleRate,
      '-b', opts.inputRaw.bitDepth,
      '-c', opts.inputRaw.numberOfChannels
    ].concat(args)
    onFormatReceived(null, opts.inputRaw)
  } else getFileFormat(opts.filename, onFormatReceived)
}
