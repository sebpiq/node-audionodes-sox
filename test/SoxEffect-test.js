...
describe('SoxEffect', function() {
  ...
  describe('_tick', function() {
    ...
    it('should open a mono file and apply the effect on it', function(done) {
      var opts = {filename: __dirname + '/sounds/steps-16b-44khz-mono.wav'}
        , soxEffect = new SoxEffect(dummyContext, opts, 'gain', 13.97847)
        , blocks = [], block
        , allAudio, sliced
        , ended = false
    ...
    it('should open a stereo file and apply the effect on it', function(done) {
      var opts = {filename: __dirname + '/sounds/steps-16b-44khz-stereo.wav'}
        , soxEffect = new SoxEffect(dummyContext, opts, 'gain', 13.97847)
        , blocks = [], block
        , allAudio, sliced
        , ended = false
    ...
    it('should open a file with different sampleRate and output should be context\'s sampleRate', function(done) {
      var helpers = require('./helpers')({approx: 0.1}) // bigger error because of resampling
        , opts = {filename: __dirname + '/sounds/steps-16b-22khz-mono.wav'}
        , soxEffect = new SoxEffect(dummyContext, opts, 'gain', 13.97847)
        , blocks = [], block
        , allAudio, sliced
        , ended = false
    ...
    it('should work with a headerless file if format is specified', function(done) {
      var inputRaw = {
          numberOfChannels: 1,
          bitDepth: 16,
          sampleRate: 44100
        }
        , opts = {filename: __dirname + '/sounds/steps-16b-44khz-mono.raw', inputRaw: inputRaw}
        , soxEffect = new SoxEffect(dummyContext, opts, 'gain', 13.97847)
        , helpers = require('./helpers')({approx: 0.01})
        , blocks = [], block, ended = false
        , allAudio, sliced
    ...
    it('should return an error if the file doesn\'t exist', function(done) {
      var soxEffect = new SoxEffect(dummyContext, {filename: '/sounds/unknown.wav'}, 'gain', 13.97847)
      soxEffect.on('error', function(err) {
        assert.ok(err)
        done()
      })
    ...
    it('should return an error if invalid arguments to sox', function(done) {
      var soxEffect = new SoxEffect(dummyContext, {filename: __dirname + '/sounds/steps-16b-44khz-stereo.wav'}, 'blablabla')
      soxEffect.on('error', function(err) {
        assert.ok(err)
        done()
      })
